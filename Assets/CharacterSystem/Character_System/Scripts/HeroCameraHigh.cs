...
public class HeroCameraHigh : MonoBehaviour
{
	...
	//Right stick rotate speed
	public float rotato = 0.6f;
...
//		}
		...
		if ( !Input.GetKey ( hKey.sprintToggleKey ) )
		{
			// Apply Y-mouse axis
			if (invertMouseY)
				yAngl += Input.GetAxisRaw ("YRotato") * ySpeed * 0.04f;
			else
				yAngl -= Input.GetAxisRaw ("YRotato") * ySpeed * 0.04f;
		}
	...
	void Orbit ()
	{
		...
		// Horizontal smooth rotation
		xAngl += Input.GetAxisRaw ("Horizontal") * xSpeed * 0.02f;
		// Vertical angle limitation
    	yAngl = ClampAngle (yAngl, minAngleY, maxAngleY);
